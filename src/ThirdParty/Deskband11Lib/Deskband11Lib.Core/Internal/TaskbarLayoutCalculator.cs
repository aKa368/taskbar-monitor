...
internal sealed class TaskbarLayoutCalculator(TaskbarWindowLocator taskbarWindowLocator, TaskbarContentHostOptions options, TaskbarButtonReader taskbarButtonReader, Func<int> getEffectiveMonitorIdentity)
{
    ...
    internal TaskbarLayoutDiagnostic LastDiagnostic { get; private set; }
    ...
    public async Task RefreshTaskbarButtonMeasurementAsync()
    {
        ...
        if (!TryGetTaskbarButtonsSearchRectangle(taskbarRectangle, rowRectangle, searchLeft, searchRight, out var taskbarButtonsSearchRectangle)) return;
        for (int attempt = 0; attempt < 4; attempt++)
        {
            await taskbarButtonReader.RefreshAsync(taskbarWindowLocator.TaskbarWindow, taskbarButtonsSearchRectangle, getEffectiveMonitorIdentity());
            if (taskbarButtonReader.TryGetCachedTaskbarButtonGeometry(taskbarWindowLocator.TaskbarWindow, taskbarButtonsSearchRectangle, getEffectiveMonitorIdentity(), out var measured) && measured.StartButton.IsValid) return;
            if (attempt < 3) await Task.Delay(150);
        }
    }
    ...
    public TaskbarLayoutSnapshot Calculate(double requestedWidth, double requestedHeight, double scaleFactor, TaskbarSlotInfo? ownSlot, IReadOnlyList<TaskbarSlotInfo> siblingSlots)
    {
        ...
        if (options.TrackTaskbarButtons && TryGetTaskbarButtonsSearchRectangle(taskbarRectangle, rowRectangle, searchLeft, searchRight, out var taskbarButtonsSearchRectangle)) taskbarButtonReader.TryGetTaskbarButtonGeometry(taskbarWindowLocator.TaskbarWindow, taskbarButtonsSearchRectangle, getEffectiveMonitorIdentity(), out geometry);

        var notificationLeft = geometry.NotificationArea.IsValid ? geometry.NotificationArea.Left : searchRight;
        int nativeSafeLeftBoundary = ResolveNativeSafeLeftBoundary(taskbarRectangle, notificationLeft);
        var alignmentDecision = geometry.StartButton.IsValid
            ? TaskbarAlignmentDetector.Detect(taskbarRectangle, geometry.StartButton, TaskbarAlignmentDetector.ReadRegistryAlignment())
            : nativeSafeLeftBoundary > taskbarRectangle.left
                ? new AlignmentDecision(TaskbarAlignment.Center, AlignmentSource.NativeExplorerFallback)
                : new AlignmentDecision(TaskbarAlignmentDetector.ReadRegistryAlignment(), AlignmentSource.RegistryFallback);
        var alignment = alignmentDecision.Alignment;
        var (areaLeft, areaRight, leftAlign) = SelectContentArea(options.Placement, alignment, taskbarRectangle, notificationLeft, geometry, nativeSafeLeftBoundary);
        LastDiagnostic = new TaskbarLayoutDiagnostic(alignment, alignmentDecision.Source, geometry, areaLeft, areaRight, leftAlign);
        ...
        var sharesRightGapWithAccountArea = alignment == TaskbarAlignment.Left && resolvedPlacement is TaskbarContentPlacement.AccountArea or TaskbarContentPlacement.BeforeNotificationArea;
        var siblingsInSameArea = FilterSiblingsInSameArea(siblingSlots, resolvedPlacement, getEffectiveMonitorIdentity(), sharesRightGapWithAccountArea);
        ...
        {
            var allocation = AllocateWidth(effectiveOwnSlot.Value, siblingsInSameArea, availableWidth, scaleFactor, sharesRightGapWithAccountArea);
    ...
    private static TaskbarContentPlacement ResolveActualPlacement(TaskbarContentPlacement requestedPlacement, TaskbarAlignment alignment, int leftAvailableWidth, int rightAvailableWidth)
    {
        return requestedPlacement switch
        {
            ...
            TaskbarContentPlacement.AccountArea => TaskbarContentPlacement.AccountArea,
    ...
    internal static List<TaskbarSlotInfo> FilterSiblingsInSameArea(IReadOnlyList<TaskbarSlotInfo> allSlots, TaskbarContentPlacement resolvedPlacement, int monitorIdentity, bool sharesRightGapWithAccountArea)
    {
        var result = new List<TaskbarSlotInfo>();
        foreach (var slot in allSlots)
        {
            if (slot.MonitorIdentity != monitorIdentity) continue;
            if (slot.ActualPlacement != resolvedPlacement && !(sharesRightGapWithAccountArea && IsRightGapPlacement(slot.ActualPlacement))) continue;
            result.Add(slot);
        }
        return result;
    }

    internal static (double Width, double Offset) AllocateWidth(TaskbarSlotInfo ownSlot, List<TaskbarSlotInfo> siblings, int totalAvailableWidth, double scaleFactor, bool oppositeEdges = false)
    {
        ...
        foreach (var slot in allSlots)
        {
            ...
            if (oppositeEdges && slot.ActualPlacement != ownSlot.ActualPlacement) continue;
    ...
    private static bool IsRightGapPlacement(TaskbarContentPlacement placement) =>
        placement is TaskbarContentPlacement.AccountArea or TaskbarContentPlacement.BeforeNotificationArea;
    ...
    private int ResolveNativeSafeLeftBoundary(RECT taskbarRectangle, int notificationLeft)
    {
        if (!TryGetWindowRectangle(taskbarWindowLocator.RebarWindow, out var contentRectangle)) return taskbarRectangle.left;
        return IsSafeNativeLeftBoundary(taskbarRectangle, contentRectangle, notificationLeft) ? contentRectangle.left : taskbarRectangle.left;
    }

    internal static bool IsSafeNativeLeftBoundary(RECT taskbarRectangle, RECT explorerContentRectangle, int notificationLeft) =>
        explorerContentRectangle.left > taskbarRectangle.left
        && explorerContentRectangle.left < notificationLeft
        && explorerContentRectangle.right > explorerContentRectangle.left
        && explorerContentRectangle.top < taskbarRectangle.bottom
        && explorerContentRectangle.bottom > taskbarRectangle.top;

    internal static (int AreaLeft, int AreaRight, bool LeftAlign) SelectContentArea(TaskbarContentPlacement placement, TaskbarAlignment alignment, RECT taskbarRectangle, int notificationLeft, TaskbarButtonGeometry geometry, int nativeSafeLeftBoundary = 0)
    {
        ...
        if (placement == TaskbarContentPlacement.AccountArea && !geometry.StartButton.IsValid)
            return nativeSafeLeftBoundary > taskbarRectangle.left && nativeSafeLeftBoundary <= notificationLeft
                ? (taskbarRectangle.left, nativeSafeLeftBoundary, true)
                : (taskbarRectangle.left, taskbarRectangle.left, true);
        ...
        var useLeftGap = placement switch
        {
            ...
            TaskbarContentPlacement.AccountArea => alignment == TaskbarAlignment.Center,
            TaskbarContentPlacement.Auto => alignment == TaskbarAlignment.Center && leftAvailableWidth > rightAvailableWidth,
            _ => false
        };

        if (!useLeftGap) return (rightGap.Left, rightGap.Right, placement == TaskbarContentPlacement.AccountArea);

        var leftAlign = placement is TaskbarContentPlacement.LeftEdge or TaskbarContentPlacement.Auto or TaskbarContentPlacement.AccountArea;
...
internal readonly record struct TaskbarLayoutDiagnostic(
    TaskbarAlignment Alignment,
    AlignmentSource AlignmentSource,
    TaskbarButtonGeometry Geometry,
    int AreaLeft,
    int AreaRight,
    bool LeftAlign)
{
    public override string ToString() => $"alignment={Alignment};source={AlignmentSource};start={Geometry.StartButton.Left},{Geometry.StartButton.Right};widgets={Geometry.WidgetsButton.Left},{Geometry.WidgetsButton.Right};buttons={Geometry.TaskbarButtonsGroup.Left},{Geometry.TaskbarButtonsGroup.Right};notification={Geometry.NotificationArea.Left},{Geometry.NotificationArea.Right};area={AreaLeft},{AreaRight};leftAlign={LeftAlign}";
}
